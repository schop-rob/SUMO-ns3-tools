    ...
    for (uint32_t i = 0; i < n->GetNDevices (); i++)
    {
        ...
        if (dev->GetInstanceTypeId () == WaveNetDevice::GetTypeId())
        {
            ...
            */
            ...
            phy->TraceConnectWithoutContext ("MonitorSnifferTx", MakeCallback(&CustomApplication::MonitorTx, this));
            phy->TraceConnectWithoutContext ("PhyTxDrop", MakeCallback(&CustomApplication::PhyTxDropTrace, this));
            phy->TraceConnectWithoutContext ("PhyRxDrop", MakeCallback(&CustomApplication::PhyRxDropTrace, this));
...
CustomApplication::BroadcastInformation()
{
    ...
    //NS_LOG_DEBUG ("Node " << GetNode()->GetId() << " is BROADCASTING a packet. " << Now ());
