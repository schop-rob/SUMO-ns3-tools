...
private:
  void SetupFixedNodes(NodeContainer& fixedNodes, const std::vector<Vector>& positions);
  ...
  static const uint32_t NUM_FIXED_NODES = 6;
};
...
void
RoutingExperiment::SetupFixedNodes(NodeContainer& fixedNodes, const std::vector<Vector>& positions)
{
  ...
  // Create an allocation of positions
  Ptr<ListPositionAllocator> positionAlloc = CreateObject<ListPositionAllocator>();
  for (const auto& pos : positionVec) {
...
RoutingExperiment::Run (int nSinks, double txp, std::string CSVfileName, std::string tcl_file)
{
  ...
  // Create Ns2NodeUtility instance
  Ns2NodeUtility ns2_utility(tcl_file);
  uint32_t nMobileNodes = ns2_utility.GetNNodes();
  double TotalTime = ns2_utility.GetSimulationTime();

  // Create nodes
  NodeContainer allNodes;
  allNodes.Create(nMobileNodes + NUM_FIXED_NODES);

  // Create separate container for fixed nodes
  NodeContainer fixedNodes;
  for (uint32_t i = nMobileNodes; i < nMobileNodes + NUM_FIXED_NODES; ++i) {
    fixedNodes.Add(allNodes.Get(i));
  }

  // Configure mobility from TCL file
  Ns2MobilityHelper mobility(tcl_file);
  mobility.Install(); // This will install mobility on the first nMobileNodes nodes
  ...
  SetupFixedNodes(fixedNodes, fixedPositions);
  ...
  NetDeviceContainer adhocDevices = wifi.Install (wifiPhy, wifiMac, allNodes);
  ...
  for (int i = 0; i < nSinks && i < (int)(nMobileNodes + NUM_FIXED_NODES); i++)
    {
      Ptr<Socket> sink = SetupPacketReceive (adhocInterfaces.GetAddress (i), allNodes.Get (i));

      AddressValue remoteAddress (InetSocketAddress (adhocInterfaces.GetAddress (i), port));
      onoff1.SetAttribute ("Remote", remoteAddress);

      // For mobile nodes, use entry time from ns2_utility
      double startTime = 0.0;
      double stopTime = TotalTime;
      
      if (i < (int)nMobileNodes) {
        startTime = ns2_utility.GetEntryTimeForNode(i);
        stopTime = ns2_utility.GetExitTimeForNode(i);
      }

      ApplicationContainer temp = onoff1.Install (allNodes.Get (i + nSinks));
  ...
  double deliveryRate = 0.0;
    ...
    std::ofstream experimentFile("/path/to/your/experiment_v2g.csv", std::ios::app);
...
main (int argc, char *argv[])
{
    ...
    std::ifstream checkFile("/path/to/your/experiment_v2g.csv");
    if (!checkFile.good()) {
        // File doesn't exist, create it with headers
        std::ofstream experimentFile("/path/to/your/experiment_v2g.csv");
