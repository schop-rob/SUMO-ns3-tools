...
RoutingExperiment::CalculateAverageDelay ()
{
  ...
  std::ofstream delayFile ("/path/to/yourdensity_delay.txt", std::ios::app);
...
RoutingExperiment::Run (int nSinks, double txp, std::string CSVfileName, std::string tcl_file)
{
  ...
  uint32_t nWifis = ns2_utility.GetNNodes();
  ...
  for (int i = 0; i < nSinks && i < (int)nWifis; i++)
    {
      ...
      // Use entry time from ns2_utility
      double startTime = ns2_utility.GetEntryTimeForNode(i);
      double stopTime = ns2_utility.GetExitTimeForNode(i);
  ...
  double deliveryRate = 0.0;
    ...
    std::ofstream experimentFile("/path/to/your/experiment.csv", std::ios::app);
...
main (int argc, char *argv[])
{
    ...
    std::ifstream checkFile("/path/to/your/experiment.csv");
    if (!checkFile.good()) {
        // File doesn't exist, create it with headers
        std::ofstream experimentFile("/path/to/your/experiment.csv");
